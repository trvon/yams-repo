...
describe("Plugin Registry API", () => {
	...
	describe("GET /api/v1/plugins", () => {
		it("should list all available plugins", async () => {
			...
			getMockBucket(env).set(
				"plugins/yams_ghidra/manifest.json",
				JSON.stringify({
					name: "yams_ghidra",
					version: "0.1.0",
			...
			getMockBucket(env).set(
				"plugins/yams_pdf/manifest.json",
				JSON.stringify({
					name: "yams_pdf",
					version: "0.2.0",
	...
	describe("GET /api/v1/plugins/:name", () => {
		it("should return plugin metadata", async () => {
			...
			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra",
			);
			const res = await worker.fetch(req, env, ctx);
		...
		it("should return 404 for non-existent plugin", async () => {
			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/nonexistent",
			);
			const res = await worker.fetch(req, env, ctx);
	...
	describe("GET /api/v1/plugins/:name/versions", () => {
		it("should list all versions of a plugin", async () => {
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.1.0/manifest.json",
				JSON.stringify({ version: "0.1.0" }),
			);
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.2.0/manifest.json",
				JSON.stringify({ version: "0.2.0" }),
			);
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.3.0/manifest.json",
				JSON.stringify({ version: "0.3.0" }),
			);

			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra/versions",
			);
			const res = await worker.fetch(req, env, ctx);
	...
	describe("GET /api/v1/plugins/:name/:version", () => {
		it("should return specific version metadata", async () => {
			...
			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra/0.1.0",
			);
			const res = await worker.fetch(req, env, ctx);
		...
		it("should return 404 for non-existent version", async () => {
			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra/99.0.0",
			);
			const res = await worker.fetch(req, env, ctx);
	...
	describe("GET /api/v1/plugins/:name/latest", () => {
		it("should redirect to latest version download", async () => {
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.1.0/manifest.json",
				JSON.stringify({ name: "yams_ghidra", version: "0.1.0" }),
			);
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.2.0/manifest.json",
				JSON.stringify({ name: "yams_ghidra", version: "0.2.0" }),
			);
			getMockBucket(env).set(
				"plugins/yams_ghidra/0.3.0/manifest.json",
				JSON.stringify({ name: "yams_ghidra", version: "0.3.0" }),
			);

			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra/latest",
			);
			const res = await worker.fetch(req, env, ctx);
	...
	describe("POST /api/v1/plugins/:name/install", () => {
		it("should track installation metrics", async () => {
			const req = createMockRequest(
				"https://repo.yams.dev/api/v1/plugins/yams_ghidra/install",
				{
					method: "POST",
	...
	describe("Cache headers", () => {
		it("should cache plugin list for short duration", async () => {
			getMockBucket(env).set(
				"plugins/yams_ghidra/manifest.json",
				JSON.stringify({ name: "yams_ghidra" }),
			);
