...
export class MockR2ObjectBody {
	...
	async json<T>(): Promise<T> {
		...
		try {
			...
			throw new SyntaxError(`Invalid mock R2 JSON body: ${String(error)}`);
		}
	}
...
export class MockR2Bucket {
	private objects: Map<
		string,
		{ content: ArrayBuffer; etag: string; uploaded?: Date }
	> = new Map();
	...
	async list(options?: {
		...
		const matchingKeys = Array.from(this.objects.keys()).filter((key) =>
		...
		for (const key of matchingKeys) {
			...
			if (delimiterIndex >= 0) {
				...
				const dirPrefix =
					prefix + relativePath.substring(0, delimiterIndex + 1);
	...
	async put(
		_key: string,
		_value: ReadableStream | ArrayBuffer | string,
	): Promise<never> {
